    ...
    @if(isset($cartItems) && count($cartItems) > 0)
        @foreach($cartItems as $item)
            ...
            <div class="flex items-center gap-4 p-4 border-b border-gray-200" data-line-id="{{ $node['id'] }}">
                ...
                <div>Rs. {{ number_format((float)$price, 0) }}</div>
                ...
                <div class="text-right min-w-[80px] font-bold">Rs. {{ number_format((float)$lineTotal, 0) }}</div>
            </div>
        @endforeach
        
        <div class="mt-8 text-right">
            <p class="text-lg font-semibold">Total: Rs. {{ number_format((float)($subtotal ?? 0), 0) }}</p>
            <a href="{{ route('checkout') }}" class="mt-4 inline-block bg-gray-900 text-white px-8 py-3 rounded-lg font-semibold hover:bg-gray-700 transition">
                Proceed to Checkout →
            </a>
        </div>
    @else
